<script lang="ts">
  ...
  async function loadLibraries() {
    try {
      ...
      // Get all libraries
      const response = await fetch(`${localStorage.getItem('plex_url')}/library/sections`, {
        headers: {
      ...
      const libraryDetails = await Promise.all(libraryIds.map((id: string) => plex.getLibrary(id)));

      libraries = libraryDetails.filter((lib): lib is Library => lib !== undefined);
    } catch (e) {
...
<div class="container mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
  ...
  {:else if libraries.length === 0}
    <div class="rounded-lg bg-surface-800 p-8 text-center">
      ...
      <h3 class="mt-2 text-sm font-medium text-surface-200">No libraries found</h3>
      <p class="mt-1 text-sm text-surface-400">
        Connect to your Plex server to view your libraries.
      </p>
      <div class="mt-6">
        <a
          href="/settings"
